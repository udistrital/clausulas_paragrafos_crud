...
@ApiTags('orden-clausulas')
@Controller('orden-clausulas')
export class OrdenClausulaController {
  constructor(private ordenClausulaService: OrdenClausulaService) {}

  @Post()
  async post(@Res() res, @Body() ordenClausulaDto: CreateOrdenClausulaDto) {
    const ordenClausula =
      await this.ordenClausulaService.post(ordenClausulaDto);
    if (!ordenClausula) {
  ...
  async put(
    @Res() res,
    @Param('id') id: string,
    @Body() ordenClausulaDto: CreateOrdenClausulaDto,
  ) {
    const ordenClausula = await this.ordenClausulaService.put(
      id,
      ordenClausulaDto,
    );
    if (!ordenClausula) {
