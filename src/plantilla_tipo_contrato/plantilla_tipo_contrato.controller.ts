...
export class PlantillaTipoContratoController {
  ...
  async post(
    ...
    try {
      const plantillaTipoContrato =
        await this.plantillaTipoContratoService.post(plantillaTipoContratoDto);
  ...
  async getAll(@Res() res, @Query() filterDto: FilterDto) {
    try {
      const { data, total } =
        await this.plantillaTipoContratoService.getAll(filterDto);
  ...
  async getById(@Res() res, @Param('id') id: string) {
    try {
      const plantillaTipoContrato =
        await this.plantillaTipoContratoService.getById(id);
      res.status(HttpStatus.OK).json({
        Success: true,
        Status: 200,
        Message: 'Request successful',
  ...
  async put(
    ...
    try {
      const plantillaTipoContrato = await this.plantillaTipoContratoService.put(
        id,
  ...
  async delete(@Res() res, @Param('id') id: string) {
    try {
      await this.plantillaTipoContratoService.delete(id);
      res.status(HttpStatus.OK).json({
        Success: true,
        Status: 200,
